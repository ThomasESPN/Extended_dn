...
class RebalancingManager:
    ...
    def _get_gas_params(self, w3):
        ...
        try:
            ...
            # Calculer maxFeePerGas avec une marge de sécurité très importante
            # Utiliser 4.0x pour gérer les fluctuations rapides du baseFee entre le calcul et l'envoi
            # Sur Arbitrum, les frais peuvent fluctuer rapidement, donc on utilise une marge généreuse
            if base_fee > 0:
                max_fee = int(base_fee * 4.0) + max_priority_fee
                logger.debug(f"Gas params: baseFee={base_fee}, maxFeePerGas={max_fee}, maxPriorityFeePerGas={max_priority_fee}")
            else:
                # Fallback si baseFee n'est pas disponible
                gas_price = w3.eth.gas_price
                max_fee = int(gas_price * 3.0)  # Marge de sécurité même en fallback
                max_priority_fee = w3.to_wei(0.1, 'gwei')
                logger.debug(f"Using fallback gas params: maxFeePerGas={max_fee}")
            ...
            # Fallback vers gasPrice legacy avec marge de sécurité
            gas_price = w3.eth.gas_price
            return {
                'gasPrice': int(gas_price * 1.5)  # Marge de sécurité même en fallback
            }
    ...
    def __init__(self, config):
        ...
        # Le seuil peut être en pourcentage (0.1 = 10%) ou en USD (250.0 = $250)
        # Si > 1, on considère que c'est en USD, sinon c'est un pourcentage
        threshold_raw = config.get('arbitrage', 'rebalance_threshold', default=0.1)
        if threshold_raw is not None and threshold_raw > 1.0:
            # Seuil en USD
            self.threshold_usd = float(threshold_raw)
            self.threshold_pct = None
            logger.debug(f"Rebalancing threshold: ${self.threshold_usd:.2f} USD")
        else:
            # Seuil en pourcentage
            self.threshold_pct = float(threshold_raw) if threshold_raw is not None else 0.1
            self.threshold_usd = None
            logger.debug(f"Rebalancing threshold: {self.threshold_pct*100:.1f}%")
        ...
        if HAS_EXTENDED_SDK:
            try:
                ...
                if api_key and public_key and private_key and vault_id:
                    ...
                    if not vault_id: missing.append('vault_id')
                    logger.warning(f"Extended API credentials not fully configured. Missing: {missing}")
            except Exception as e:
    ...
    def check_balance_needed(self) -> Dict[str, Dict[str, float]]:
        ...
        """
        try:
            logger.debug("Vérification des balances pour le rebalancing...")
            
            # Récupérer les balances
            ext_balance = self._get_balance("Extended")
            hyp_balance = self._get_balance("Hyperliquid")
            
            if ext_balance == 0.0 and hyp_balance == 0.0:
                logger.warning("⚠️  Impossible de récupérer les balances (toutes à 0)")
                return {}
            
            total = ext_balance + hyp_balance
            
            if total == 0:
                logger.warning("⚠️  Total des balances = 0, impossible de calculer le rebalancing")
                return {}
            ...
            logger.debug(f"Balances: Extended=${ext_balance:.2f}, Hyperliquid=${hyp_balance:.2f}, Total=${total:.2f}")
            logger.debug(f"Idéal par exchange: ${ideal_per_exchange:.2f}")
            logger.debug(f"Différences: Extended=${ext_diff:.2f}, Hyperliquid=${hyp_diff:.2f}")
            
            # Vérifier si le seuil est dépassé
            # Utiliser le seuil en USD si disponible, sinon utiliser le pourcentage
            if self.threshold_usd is not None:
                # Seuil en USD : comparer la différence absolue entre les deux comptes
                # (cohérent avec le calcul dans dnfarming.py: abs(bal1 - bal2))
                absolute_diff = abs(ext_balance - hyp_balance)
                needs_rebalancing = absolute_diff > self.threshold_usd
                logger.info(f"Seuil USD: ${self.threshold_usd:.2f}, Différence absolue: ${absolute_diff:.2f}")
            else:
                # Seuil en pourcentage : comparer les pourcentages par rapport à l'idéal
                ext_diff_pct = abs(ext_diff) / total if total > 0 else 0
                hyp_diff_pct = abs(hyp_diff) / total if total > 0 else 0
                max_diff_pct = max(ext_diff_pct, hyp_diff_pct)
                needs_rebalancing = max_diff_pct > self.threshold_pct
                logger.info(f"Seuil %: {self.threshold_pct*100:.1f}%, Différence max: {max_diff_pct*100:.1f}%")
            ...
            if needs_rebalancing:
                ...
                if ext_diff > 0:
                    ...
                    transfer_amount = abs(ext_diff)
                    result['recommended_transfer'] = {
                        'from': 'Extended',
                        'to': 'Hyperliquid',
                        'amount': transfer_amount
                    }
                    logger.info(f"Transfert recommandé: Extended -> Hyperliquid, ${transfer_amount:.2f}")
                else:
                    # Hyperliquid a trop, transférer vers Extended
                    transfer_amount = abs(hyp_diff)
                    result['recommended_transfer'] = {
                        'from': 'Hyperliquid',
                        'to': 'Extended',
                        'amount': transfer_amount
                    }
                    logger.info(f"Transfert recommandé: Hyperliquid -> Extended, ${transfer_amount:.2f}")
            else:
                logger.debug("Pas de rebalancing nécessaire")
    ...
    def deposit_hyperliquid(self, amount: float) -> Dict:
        ...
        try:
            ...
            # Obtenir les paramètres de gas EIP-1559 (récupéré juste avant l'envoi pour éviter les problèmes de timing)
            gas_params = self._get_gas_params(w3)
            
            # Construire la transaction transfer avec les paramètres EIP-1559
        ...
        try:
            ...
            async def _async_withdraw():
                ...
                try:
                    ...
                    # Arrondir le montant à 2 décimales pour éviter les erreurs de précision
                    ...
                    # Utiliser le montant exact du quote pour le withdrawal (plus sûr)
                    # Le quote peut avoir ajusté le montant légèrement
                    quote_amount = Decimal(str(quote.amount)) if hasattr(quote, 'amount') else amount_decimal
                    logger.debug(f"Quote amount: {quote_amount}, Original amount: {amount_decimal}")
                    ...
                    # Augmenter le délai pour éviter les erreurs 500 (quote pas encore prêt)
                    ...
                    try:
                        ...
                        # Utiliser le montant exact du quote si disponible, sinon arrondir à 2 décimales
                        # Le montant doit correspondre exactement au quote pour éviter les erreurs 500
                        if hasattr(quote, 'amount') and quote.amount:
                            amount_for_withdrawal = Decimal(str(quote.amount))
                            logger.debug(f"Using quote amount: {amount_for_withdrawal}")
                        else:
                            # Fallback: arrondir le montant à 2 décimales
                            amount_for_withdrawal = Decimal(str(round(float(amount_decimal), 2)))
                            logger.debug(f"Using rounded amount: {amount_for_withdrawal}")
                        ...
                        if '500' in error_str or 'Internal Server Error' in error_str:
                            ...
                            logger.error("   - Quote ID pas encore prêt (délai insuffisant)")
                            logger.error(f"   Quote ID utilisé: {quote.id}")
                            logger.error(f"   Montant original: ${amount:.2f} USD")
                            logger.error(f"   Montant arrondi: {amount_for_withdrawal} USD")
                            logger.error(f"   Montant du quote: {amount_decimal} USD")
    ...
    def execute_rebalancing(
        ...
        """
        Exécute un transfert de rebalancing entre Extended et Hyperliquid via Arbitrum
        
        Args:
            from_exchange: Exchange source ("Extended" ou "Hyperliquid")
            to_exchange: Exchange destination ("Extended" ou "Hyperliquid")
            amount: Montant à transférer en USDC
        ...
        try:
            ...
            if from_exchange == "Extended" and to_exchange == "Hyperliquid":
                # Cas 1: Extended -> Hyperliquid
                return self._rebalance_extended_to_hyperliquid(amount)
            elif from_exchange == "Hyperliquid" and to_exchange == "Extended":
                # Cas 2: Hyperliquid -> Extended
                return self._rebalance_hyperliquid_to_extended(amount)
            else:
                logger.error(f"Combinaison d'exchanges non supportée: {from_exchange} -> {to_exchange}")
                return False
    ...
    def _rebalance_extended_to_hyperliquid(self, amount: float) -> bool:
        """
        Rebalance depuis Extended vers Hyperliquid
        
        Processus:
        1. Withdraw depuis Extended vers Arbitrum
        2. Attendre que les fonds arrivent sur Arbitrum
        3. Deposit depuis Arbitrum vers Hyperliquid
        ...
        """
        try:
            logger.info(f"📤 Étape 1: Retrait de ${amount:.2f} depuis Extended vers Arbitrum...")
            
            # Retirer depuis Extended
            withdraw_result = self.withdraw_extended(amount)
            
            if withdraw_result.get('status') != 'success':
                error_msg = withdraw_result.get('message', 'Unknown error')
                logger.error(f"❌ Échec du retrait Extended: {error_msg}")
                return False
            
            logger.success(f"✅ Retrait Extended initié: {withdraw_result.get('withdrawal_id', 'N/A')}")
            bridge_fee = withdraw_result.get('bridge_fee', 0)
            expected_amount = amount - bridge_fee if bridge_fee > 0 else amount * 0.9975  # Estimation -0.25%
            
            logger.info(f"⏳ Attente de la finalisation du bridge Extended (environ 5-10 minutes)...")
            logger.info(f"   Montant retiré: ${amount:.2f}")
            ...
            if not self.hyperliquid_wallet:
                logger.error("Hyperliquid wallet not initialized - cannot check Arbitrum balance")
                return False
            
            arbitrum_address = self.hyperliquid_wallet.address
            min_expected = expected_amount * 0.95  # 95% du montant attendu
            
            logger.info(f"Vérification du solde USDC sur Arbitrum pour {arbitrum_address}...")
            
            # Attendre jusqu'à 10 minutes
            if not self._wait_for_arbitrum_balance(arbitrum_address, min_expected, max_wait_seconds=600):
                logger.error("❌ Le retrait Extended n'a pas été finalisé dans les temps")
                return False
            
            # Récupérer le solde réel
            actual_balance = self._get_arbitrum_balance(arbitrum_address)
            if actual_balance > 0:
            ...
            # Vérifier le minimum pour Hyperliquid (5 USDC)
            if deposit_amount < self.min_deposit_amount:
                logger.error(f"❌ Montant trop faible pour Hyperliquid: ${deposit_amount:.2f} < ${self.min_deposit_amount:.2f}")
                return False
            
            logger.info(f"📥 Étape 2: Dépôt de ${deposit_amount:.2f} depuis Arbitrum vers Hyperliquid...")
            
            # Déposer vers Hyperliquid
            deposit_result = self.deposit_hyperliquid(deposit_amount)
            
            if deposit_result.get('status') != 'success':
                error_msg = deposit_result.get('message', 'Unknown error')
                logger.error(f"❌ Échec du dépôt Hyperliquid: {error_msg}")
                return False
            
            logger.success(f"✅ Dépôt Hyperliquid réussi: {deposit_result.get('transaction_hash', 'N/A')}")
            logger.info("⏳ Le dépôt sera crédité sur votre compte Hyperliquid dans moins d'1 minute")
            
            return True
            
        except Exception as e:
            logger.error(f"❌ Erreur lors du rebalancing Extended -> Hyperliquid: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return False
    
    def _rebalance_hyperliquid_to_extended(self, amount: float) -> bool:
        """
        Rebalance depuis Hyperliquid vers Extended
        
        Processus:
        1. Withdraw depuis Hyperliquid vers Arbitrum
        2. Attendre que les fonds arrivent sur Arbitrum
        3. Deposit depuis Arbitrum vers Extended
        ...
        """
        try:
            logger.info(f"📤 Étape 1: Retrait de ${amount:.2f} depuis Hyperliquid vers Arbitrum...")
            
            # Récupérer l'adresse Arbitrum de destination (même wallet généralement)
            if not self.hyperliquid_wallet:
                logger.error("Hyperliquid wallet not initialized")
                return False
            
            destination_address = self.hyperliquid_wallet.address
            
            # Retirer depuis Hyperliquid (frais de $1)
            withdraw_result = self.withdraw_hyperliquid(amount, destination_address)
            ...
            logger.success(f"✅ Retrait Hyperliquid initié vers {destination_address}")
            ...
            expected_amount = amount - 1.0  # Frais Hyperliquid de $1
            
            # Attendre que les fonds arrivent sur Arbitrum
            min_expected = expected_amount * 0.95  # 95% du montant attendu
            
            logger.info(f"Vérification du solde USDC sur Arbitrum pour {destination_address}...")
            
            # Attendre jusqu'à 10 minutes
            if not self._wait_for_arbitrum_balance(destination_address, min_expected, max_wait_seconds=600):
                logger.error("❌ Le retrait Hyperliquid n'a pas été finalisé dans les temps")
                return False
            
            # Récupérer le solde réel
            actual_balance = self._get_arbitrum_balance(destination_address)
            if actual_balance > 0:
                deposit_amount = actual_balance
                logger.info(f"✅ Solde disponible sur Arbitrum: ${deposit_amount:.2f} USDC")
            else:
            ...
            # Déposer vers Extended
            deposit_result = self.deposit_extended(deposit_amount)
            
            if deposit_result.get('status') != 'success':
                error_msg = deposit_result.get('message', 'Unknown error')
                logger.error(f"❌ Échec du dépôt Extended: {error_msg}")
                return False
            ...
            logger.error(f"❌ Erreur lors du rebalancing Hyperliquid -> Extended: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return False
    ...
    def _wait_for_arbitrum_balance(self, address: str, min_balance: float, max_wait_seconds: int = 600) -> bool:
        """
    ...
    def auto_rebalance_if_needed(self) -> bool:
        ...
        try:
            ...
            if not check:
                logger.error("❌ Impossible de vérifier les balances pour le rebalancing")
                return False
            
            if not check.get('needs_rebalancing'):
                logger.debug("No rebalancing needed")
                return False
            
            if not check.get('recommended_transfer'):
                logger.warning("⚠️  Rebalancing nécessaire mais aucun transfert recommandé")
                return False
            
            transfer = check['recommended_transfer']
            
            logger.info("🔄 Auto-rebalancing déclenché")
            logger.info(f"  De: {transfer['from']}")
            logger.info(f"  Vers: {transfer['to']}")
            logger.info(f"  Montant: ${transfer['amount']:.2f}")
            ...
            logger.error(f"❌ Erreur lors de la vérification du rebalancing: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return False
    ...
    def _get_balance(self, exchange: str) -> float:
        ...
        try:
            if exchange == "Hyperliquid":
                balance = self._get_hyperliquid_balance()
                logger.debug(f"Balance Hyperliquid: ${balance:.2f}")
                return balance
            elif exchange == "Extended":
                balance = self._get_extended_balance()
                logger.debug(f"Balance Extended: ${balance:.2f}")
                return balance
            else:
    ...
    def _get_hyperliquid_balance(self) -> float:
        ...
        if not self.hyperliquid_client:
            logger.warning("Hyperliquid client not initialized in RebalancingManager")
            logger.warning("Vérifiez que wallet_address est bien configuré")
            return 0.0
        
        if not self.wallet_address:
            logger.warning("Wallet address not configured in RebalancingManager")
            return 0.0
    ...
    def _get_extended_balance(self) -> float:
        ...
        if not self.extended_account:
            logger.warning("Extended account not initialized in RebalancingManager")
            logger.warning("Vérifiez que les credentials Extended sont bien configurés")
            return 0.0
